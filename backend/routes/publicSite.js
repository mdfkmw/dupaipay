...
router.post('/orders/:orderId/start-payment', async (req, res) => {
  ...
  try {
    ...
    if (order.status !== 'pending') {
      conn.release();
      return res.status(409).json({ error: 'Comanda nu este în stare pending.' });
    }
    ...
    console.log('[ipay/start-payment] order', { orderId, operatorId, amountMinor, returnUrl });
    ...
    conn.release();
    console.error('[public/start-payment] error', err);
...
const { rows } = await db.query(
  ...
  console.log('[email] order row:', rows?.[0]);

  const order = rows?.[0];

  if (order && !order.email_sent_at && order.customer_email) {
    const receiptResp = await fetch(
      `${process.env.PUBLIC_SITE_BASE_URL}/api/public/orders/${orderId}/receipt`
    );
  ...
  console.error('[email] failed to send confirmation', err);
    ...
    return res.status(500).send('Eroare la confirmarea platii.');
  }
});
...
LIMIT 1
    ...
    if (!orderRows.length) return res.status(404).json({ error: 'Comanda nu există.' });
    ...
    // === Ora corecta in statia de urcare (board_station_id) ===
const isRetur = String(order.direction || '').toLowerCase() === 'retur';

// route_stations: sequence + travel_time_from_previous_minutes
const { rows: rsRows } = await db.query(
  `
    SELECT station_id, travel_time_from_previous_minutes, sequence
    FROM route_stations
    WHERE route_id = ?
    ORDER BY sequence ASC
  `,
...
// addMinutesToTime exista deja in publicSite.js (o folosesti la search)
const departure_time_correct = addMinutesToTime(order.departure_time, boardOffsetMinutes);




    const { rows: items } = await db.query(
      `SELECT seat_id, traveler_name, discount_type_id, discount_amount, promo_code_id, promo_discount_amount, price_amount FROM order_items WHERE order_id = ? ORDER BY id`,
      [orderId],
    );

    const { rows: reservations } = await db.query(
      `SELECT reservation_id AS id FROM payments WHERE order_id = ? AND reservation_id IS NOT NULL ORDER BY id`,
      [orderId],
    );
    ...
    res.status(500).json({ error: 'Eroare la încărcarea detaliilor comenzii.' });
  }
});
